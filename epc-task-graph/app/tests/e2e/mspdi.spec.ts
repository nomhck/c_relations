...
test("MSPDI: 出力→ファイル取込 で往復（新プロジェクトとして復元）", async ({
  page,
}) => {
  await page.goto("/");
  ...
  await page.evaluate(() => {
    ...
    const a = app.addTask(
      { wbsCode: "77", name: "MSPXTASK", durationDays: 6 },
      { edit: false },
    );
    const b = app.addTask(
      { wbsCode: "77", name: "MSPYTASK", durationDays: 2 },
      { edit: false },
    );
    app.addDependencyChecked(a.id, b.id);
    app.setSelection({ taskId: null });
  });
  const beforeProjectId = await page.evaluate(
    () => (window as any).__APP.getState().project.id,
  );
  ...
  const [download] = await Promise.all([
    ...
    page.getByRole("button", { name: /MS Project XMLを書き出す/ }).click(),
  ]);
  ...
  await expect
    .poll(() =>
      page.evaluate(() => (window as any).__APP.getState().project.id),
    )
  ...
  const state = await page.evaluate(() => {
    ...
    return {
      hasX: s.tasks.some(
        (t: any) => t.name === "MSPXTASK" && t.durationDays === 6,
      ),
