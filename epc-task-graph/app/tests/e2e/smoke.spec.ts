...
test("起動→タスク作成→後続接続→リロード後も残存する", async ({ page }) => {
  ...
  await dialog.getByLabel("タスク名").fill("E2E作成タスク");
  await dialog
  ...
  await page.evaluate(() => {
    const s = (window as any).__APP.getState();
    const t = s.tasks.find((t: any) => t.name === "E2E作成タスク");
    s.setSelection({ taskId: t.id });
  });
  await page.locator(".viewtabs-bar").click({ position: { x: 5, y: 5 } });
  await expect
    .poll(async () => (await counts(page)).tasks)
    .toBe(base.tasks + 1);
  ...
  const hasName = await page.evaluate(() =>
    (window as any).__APP
      .getState()
      .tasks.some((t: any) => t.name === "E2E作成タスク"),
  );
  expect(hasName).toBe(true);
});

test("4,000デモ生成→ISOLATEフィルタで可視数が大幅に減る（§10 受入(e)）", async ({
  page,
}) => {
  await page.goto("/");
  await openEditor(page);
  ...
  await expect
    .poll(async () => await page.locator(".agg-card").count())
    .toBeGreaterThan(10);
...
test("CPのみ表示で背骨チェーンが抽出される（§9.2 / PR4）", async ({ page }) => {
  ...
  await page.evaluate(async () => {
    ...
    const A = s().addTask(
      { name: "A", durationDays: 2, position: { x: 0, y: 0 } },
      { edit: false },
    );
    const B = s().addTask(
      { name: "B", durationDays: 4, position: { x: 220, y: 0 } },
      { edit: false },
    );
    const C = s().addTask(
      { name: "C", durationDays: 2, position: { x: 220, y: 160 } },
      { edit: false },
    );
    const D = s().addTask(
      { name: "D", durationDays: 1, position: { x: 440, y: 0 } },
      { edit: false },
    );
  ...
  await page
    .getByRole("button", { name: "プロジェクト概要", exact: true })
    .click();
  await expect(page.locator(".date-value")).toContainText(/\d+\/\d+/);
});
