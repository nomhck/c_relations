...
function depsLen(page: Page) {
  return page.evaluate(
    () => (window as any).__APP.getState().dependencies.length,
  );
}
...
test("依存: 右パネルの検索コンボで後続を追加→×で削除", async ({ page }) => {
  ...
  await page.getByRole("tab", { name: /つながり/ }).click();
  ...
  const created = await page.evaluate(
    (p) =>
      (window as any).__APP
        .getState()
        .dependencies.some(
          (d: any) => d.predecessorId === p[0] && d.successorId === p[1],
        ),
    ids,
  );
  ...
  // 後続一覧の × で削除 → 元に戻る。
  await page
    .locator(".relation-item", { hasText: "DEP先B" })
    .getByRole("button", { name: "依存関係を削除" })
    .click();
  await expect.poll(async () => await depsLen(page)).toBe(before);
});

test("依存: 循環になる接続はコンボから追加できない（拒否トースト）", async ({
  page,
}) => {
  await page.goto("/");
  await openEditor(page);
...
test("依存タイプ: FS→SS 変更で後続の ES が再計算される（Phase2 CPM）", async ({
  page,
}) => {
  await page.goto("/");
  await openEditor(page);
  ...
  await page.evaluate(() =>
    (window as any).__APP
      .getState()
      .updateCalendar({ workingDays: [0, 1, 2, 3, 4, 5, 6] }),
  );
  ...
  await expect(page.getByTestId("cpm-es")).toHaveAttribute("data-offset", "4");

  // 右パネルの依存タイプ select を SS に変更 → B.ES = ES_A = +0d に再計算。
  await page.getByRole("tab", { name: /つながり/ }).click();
  await page
    .locator(".relation-item", { hasText: "PA" })
    .locator(".deptype")
    .selectOption("SS");
  await page.getByRole("tab", { name: "日程", exact: true }).click();
  await expect(page.getByTestId("cpm-es")).toHaveAttribute("data-offset", "0");
});

test("日付制約 SNET: 右パネルで設定すると ES がその日以降へ丸められる（Phase2）", async ({
  page,
}) => {
  await page.goto("/");
  await openEditor(page);
  ...
  await expect(page.getByTestId("cpm-es")).toHaveAttribute("data-offset", "7");
});
