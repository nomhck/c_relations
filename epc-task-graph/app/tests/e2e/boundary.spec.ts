...
test("担当＋前後: 世代ステッパで受け渡し先の表示範囲が広がる", async ({
  page,
}) => {
  await page.goto("/");
  ...
  await page.evaluate(() => {
    ...
    const A = app.addTask(
      { name: "A", assignee: "他", wbsCode: "1" },
      { edit: false },
    );
    const B = app.addTask(
      { name: "B", assignee: "自", wbsCode: "1" },
      { edit: false },
    );
    const C = app.addTask(
      { name: "C", assignee: "他", wbsCode: "1" },
      { edit: false },
    );
    const D = app.addTask(
      { name: "D", assignee: "他", wbsCode: "1" },
      { edit: false },
    );
  ...
  const setBoundary = (up: number, down: number) =>
    page.evaluate(
      ([u, d]) => (window as any).__APP.getState().setBoundary(u, d),
      [up, down],
    );
  ...
  const taskNames = async () =>
    page.evaluate(() =>
      Array.from(
        document.querySelectorAll(".trow:not(.trow-wbs) .tname-text"),
      ).map((e) => e.textContent),
    );
  ...
  await expect
    .poll(async () => (await taskNames()).sort().join(","))
    .toBe("A,B,C");
