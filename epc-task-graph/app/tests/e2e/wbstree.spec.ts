...
function wbsFilter(page: Page) {
  return page.evaluate(
    () => (window as any).__APP.getState().viewSpec.filter.wbsPrefixes ?? null,
  );
}

test("WBSツリー: 展開してノードクリックで wbsPrefixes 絞り込み（再クリックで解除）", async ({
  page,
}) => {
  await page.goto("/");
  ...
  await tree
    .locator('.wbs-tree-row:has([data-prefix="1"]) .wbs-tree-tog')
    .click();
