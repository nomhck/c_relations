...
test("稼働カレンダー: 週休2日で週末を跨ぐ作業が、土日を稼働にすると短縮される", async ({
  page,
}) => {
  await page.goto("/");
  ...
  await expect(page.getByTestId("cpm-ef")).toHaveAttribute("data-offset", "8");
  ...
  await expect(page.getByTestId("cpm-ef")).toHaveAttribute("data-offset", "6");
});
