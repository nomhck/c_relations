...
test("ガント依存矢印: 表示中ペアに矢印を描き、依存線トグルで消える", async ({
  page,
}) => {
  await page.goto("/");
  ...
  await page.evaluate(() => {
    ...
    const a = app.addTask(
      { wbsCode: "80", name: "AR-A", durationDays: 5 },
      { edit: false },
    );
    const b = app.addTask(
      { wbsCode: "80", name: "AR-B", durationDays: 4 },
      { edit: false },
    );
    const c = app.addTask(
      { wbsCode: "80", name: "AR-C", durationDays: 3 },
      { edit: false },
    );
  ...
  // 依存矢印が描画される（少なくとも2本）。
  await expect
    .poll(async () => await page.locator(".gantt-dep-line").count())
