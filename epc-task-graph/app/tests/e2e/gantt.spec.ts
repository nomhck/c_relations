...
test("ガント: Yショートカット→軸/バー描画→CPのみで全バー critical", async ({
  page,
}) => {
  await page.goto("/");
  ...
  await expect
    .poll(async () =>
      page.evaluate(() => (window as any).__APP.getState().activeView),
    )
    .toBe("gantt");
  ...
  await expect(page.locator(".gantt-tick > span").first()).toHaveText(
    /\d{4}-\d{2}/,
  );

  // タスクバー（CPM es/ef 由来）が描画される
  await expect
    .poll(async () => await page.locator(".gantt-bar").count())
    .toBeGreaterThan(1);

  // CPのみ表示 → 表示中のバーはすべて critical（背骨だけ残る）
  await page.evaluate(() =>
    (window as any).__APP.getState().quickCriticalOnly(),
  );
  await expect
    .poll(async () => {
