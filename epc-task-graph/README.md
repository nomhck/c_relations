...
**多ビュー3タブが全て稼働**（設計書 §12）:
- **グラフ**（`G`）: React Flow・表示パイプライン（フィルタDIM/ISOLATE・WBS折り畳み・近傍フォーカス）・4,000ノード対応。俯瞰は集約カード＋WBSグリッド整列で判読可能。
- **テーブル**（`T`）: 仮想スクロール・インライン編集・多重ソート・複数行選択＋一括操作（削除/ステータス/工種/担当）・WBS日付集計・wbsCode編集・`Tab`後続作成。
- **ガント**（`Y`）: 左=タスク名列／右=時間軸（月目盛り＋基準日ライン）、バー=CPM es/ef・WBSサマリバー・工種色分け・CP赤・進捗オーバレイ。行仮想化はテーブルと共用。

基盤: CPM Step1（暦日・FS）＋`selectCpm`メモ化・Dexie差分永続化・複数プロジェクト・zundo差分Undo/Redo。テスト: domain単体54＋e2e（smoke/table/gantt）。

次候補: 保存ビューUI（→SavedViewへのソート/列保存 PR-T2④）／ Phase2 CPM完成（依存タイプ・カレンダー・制約）／ 関係タスクのハイライト強化・世代フィルタ・GUI依存接続改善。
